...
- **Source:** BIDS Contributors. *Brain Imaging Data Structure 1.11.1*. [Official version-pinned specification](https://bids-specification.readthedocs.io/en/v1.11.1/), [common principles](https://bids-specification.readthedocs.io/en/v1.11.1/common-principles.html), [dataset description](https://bids-specification.readthedocs.io/en/v1.11.1/modality-agnostic-files/dataset-description.html), [events](https://bids-specification.readthedocs.io/en/v1.11.1/modality-agnostic-files/events.html), and [EEG specification](https://bids-specification.readthedocs.io/en/v1.11.1/modality-specific-files/electroencephalography.html).
...
- **Limits:** BIDS describes representation and required/recommended metadata, not the scientific validity of a preprocessing choice. The version-pinned documentation is remote rather than bundled; when it is inaccessible, defer unresolved normative questions instead of substituting memory or a moving documentation version. Record both the declared BIDS version and validator version.
...
- **Source:** BIDS Contributors. *Brain Imaging Data Structure 1.11.1*. [Official version-pinned derivatives introduction](https://bids-specification.readthedocs.io/en/v1.11.1/derivatives/introduction.html) and [common derivative data types](https://bids-specification.readthedocs.io/en/v1.11.1/derivatives/common-data-types.html).
...
<a id="s62"></a>
## S62 — EEGLAB STUDY construction contract

- **Type / class:** Official tutorial, version-pinned source, and bounded execution on EEGLAB 2026.0.0; `SOFTWARE_CONTRACT`.
- **Source:** SCCN. *Creating an EEGLAB STUDY* and *Command-line STUDY functions*. [Official STUDY creation tutorial](https://eeglab.org/tutorials/10_Group_analysis/study_creation.html), [official scripting tutorial](https://eeglab.org/tutorials/11_Scripting/command_line_study_functions.html), and EEGLAB commit `bcc710a8edd712738e48879b6846958a2be7be1d` [`std_editset.m`](https://github.com/sccn/eeglab/blob/bcc710a8edd712738e48879b6846958a2be7be1d/functions/studyfunc/std_editset.m).
- **Supports:** A STUDY is an EEGLAB-native multi-dataset structure containing dataset descriptions/links and study-level information. The official tutorial creates one from loaded datasets or files. The pinned `std_editset` command contract supports `index`, `load`, `subject`, `condition`, `session`, `group`, `task`, and `run`; when initialized from `ALLEEG`, it populates corresponding `STUDY.datasetinfo` fields and keeps dataset metadata synchronized. A bounded run created a STUDY from two synthetic SET files and verified dataset count, paths, and subject/session/run/task identities.
- **Limits:** STUDY fields are EEGLAB representations, not BIDS conformance or lossless publisher metadata. `datasetinfo` does not establish the truth of subject, condition, event, or acquisition semantics. Saving behavior, accepted text types, File-IO support, and update rules are release-specific; inspect the active source and output tree.

<a id="s63"></a>
## S63 — EEG-BIDS import and metadata-access contract

- **Type / class:** Official version-pinned source plus bounded execution on EEG-BIDS 10.5, EEGLAB 2026.0.0, and MATLAB R2026a Update 4; `SOFTWARE_CONTRACT`.
- **Source:** SCCN. EEG-BIDS commit `8486b1dde369079578452e632ee1aac29fe86db8` [`pop_importbids.m`](https://github.com/sccn/EEG-BIDS/blob/8486b1dde369079578452e632ee1aac29fe86db8/pop_importbids.m); EEGLAB commit `bcc710a8edd712738e48879b6846958a2be7be1d` [`pop_loadset.m`](https://github.com/sccn/eeglab/blob/bcc710a8edd712738e48879b6846958a2be7be1d/functions/popfunc/pop_loadset.m).
- **Supports:** The pinned importer reads dataset-level descriptions, participants, recording sidecars, channels, electrodes, events, coordinate systems, and selected auxiliary files, then translates them into EEGLAB structures. It sets selected recording identity fields; constructs `EEG.BIDS` fields `gInfo`, `pInfo`, `pInfoDesc`, `eInfo`, `eInfoDesc`, `tInfo`, `bidsstats`, `scannedElectrodes`, and `behavioral`; and builds `std_editset` commands from subject/session/task/run plus participant and first behavioral-record fields. Full import creates EEGLAB SET/FDT/STUDY products, not byte-for-byte copies of BIDS JSON/TSV sidecars. In `metadata='on'`, SET uses `pop_loadset(...,'info')` and BrainVision uses `pop_loadbv(...,true)`, while EDF/BDF explicitly uses full `pop_biosig` because events may reside in a channel; FIF/GZ/DS/MEFD branches also lack a metadata-specific call. An earlier bounded active-session import of selected ds003061 data reproduced these structures and outputs without changing the source inventory. Synthetic SET tests showed modern one-file sentinel behavior and a two-file relative-FDT working-directory fallback that can materialize data.
- **Limits:** This is the exact 10.5 implementation, not a promise for other releases or every format/plugin. A later clean-path rerun on R2026a Update 4 stopped before JSON parsing because the active MATLAB `fileread` rejected the character-vector path passed by EEG-BIDS; the earlier import had not captured that core-function path state and is therefore environment-contingent. Do not shadow `fileread` to bypass this gate: use a compatible MATLAB/EEG-BIDS release or an upstream fix and rerun. `metadata='on'` suppresses EEGLAB output/STUDY creation but does not guarantee zero sample access. Imported fields are normalized/transformed structures and can omit sidecar syntax, inheritance provenance, extra rows, or unsupported metadata. File inventories do not prove bitwise immutability; preserve and validate the BIDS source independently.

<a id="s64"></a>
## S64 — EEGLAB extension distribution and importer assets

- **Type / class:** Official extension documentation/registry, version-pinned source, retrieved release assets, and bounded execution; `SOFTWARE_CONTRACT`.
- **Source:** SCCN. [EEGLAB Extensions](https://eeglab.org/others/EEGLAB_Extensions.html), [current extension registry](https://sccn.ucsd.edu/eeglab/plugin_uploader/plugin_list_all.php), [official old-version archive](https://sccn.ucsd.edu/eeglab/plugins/), and EEGLAB commit `bcc710a8edd712738e48879b6846958a2be7be1d` [`plugin_install.m`](https://github.com/sccn/eeglab/blob/bcc710a8edd712738e48879b6846958a2be7be1d/functions/adminfunc/plugin_install.m). SCCN. bva-io commit [`d5fe22f69aadc40139e916067ca63cb52f79978a`](https://github.com/sccn/bva-io/tree/d5fe22f69aadc40139e916067ca63cb52f79978a). BioSig Project. [Official project page](https://biosig.sourceforge.net/).
- **Supports:** EEGLAB documents extension-manager installation, manual versioned-ZIP extraction, and a separate archive for old releases; its registry lists BIOSIG for broad format import and bva-io for BrainVision. `plugin_install` downloads the registry asset into the selected EEGLAB installation and unpacks it below `plugins/`, so command-line installation is a filesystem/network mutation. On 2026-08-09, `plugin_getweb` returned Biosig 3.8.5, bva-io 1.75, and GEDAI 1.5 ZIP URLs. Official HTTPS downloads produced SHA-256 `44A5D43A22F5F2F2945E5C0AA720EA7A85E90A2A3F0F6200F15F176D571CDBDA` for `Biosig3.8.5.zip` and `C3B0B6791759FEDE37B2984C7283C63A1CFD2094518B3AE2591C57EBACFFED6D` for `bva-io1.75.zip`. The bva ZIP's ten files matched SCCN Git commit `d5fe22f...` after line-ending normalization, while its archive root remained `bva-io-master`; the source route and distribution asset therefore remain distinct provenance. A clean probe found core `pop_biosig`/`pop_fileio` wrappers while BIOSIG `sopen` and plugin records were absent, showing that wrapper presence is not backend capability. A disposable archive-backed MATLAB run then resolved BIOSIG `pop_biosig`/`sopen` and used bva-io 1.75 to write a synthetic BrainVision triplet and return empty `EEG.data` with correct metadata dimensions.
- **Limits:** Not every registry plugin has an SCCN Git repository, tag corresponding to the ZIP, checksum, or Git-compatible upstream; a repository must not be invented. Registry entries and ratings are discovery metadata, not scientific review—the official extension page explicitly disclaims assessment. Blind recursive path addition of the BIOSIG archive exposed compatibility/shadowing risk; use the plugin registrar or reviewed subpaths. In the bounded R2026a/bva-io test, string paths succeeded while character-vector paths failed active `fileread` validation, so text-type workarounds are function/release-specific. Dynamic registry URLs, versions, assets, licenses, compiled dependencies, and platform compatibility require re-verification.

<a id="s65"></a>
## S65 — GEDAI method and headless API contract

- **Type / class:** Primary preprint, official upstream source, registry metadata, and bounded execution; `CONDITIONAL_EVIDENCE`, `SOFTWARE_CONTRACT`.
- **Source:** Ros T, Férat V, Huang Y, et al. *Return of the GEDAI: Unsupervised EEG Denoising based on Leadfield Filtering*. bioRxiv, 2025. [doi:10.1101/2025.10.04.680449](https://doi.org/10.1101/2025.10.04.680449). NeuroTuning. GEDAI upstream commit [`c48f7d635d2c5607d479e889ac9fd2d3bc3f29dc`](https://github.com/neurotuning/GEDAI-master/tree/c48f7d635d2c5607d479e889ac9fd2d3bc3f29dc), including [`GEDAI.m`](https://github.com/neurotuning/GEDAI-master/blob/c48f7d635d2c5607d479e889ac9fd2d3bc3f29dc/GEDAI.m), [`pop_GEDAI.m`](https://github.com/neurotuning/GEDAI-master/blob/c48f7d635d2c5607d479e889ac9fd2d3bc3f29dc/pop_GEDAI.m), and [PolyForm Noncommercial license](https://github.com/neurotuning/GEDAI-master/blob/c48f7d635d2c5607d479e889ac9fd2d3bc3f29dc/LICENSE).
- **Supports:** GEDAI uses generalized eigendecomposition with a theoretical/reference covariance based on an EEG forward-model leadfield and reports SENSAI/ENOVA-related outputs. The pinned source accepts precomputed standard-location BEM, interpolated-coordinate, template FieldTrip/DIPFIT warped, or custom covariance routes; it can auto-select thresholds and optionally reject channels/epochs. At the pinned upstream commit the registrar reports v1.7 while the EEGLAB registry reported v1.5, demonstrating release drift. `pop_GEDAI` parses `varargin` but unconditionally calls `inputgui`; the public `GEDAI` function is the parameterized computational entry point. A bounded call on the bundled example SET completed headlessly with visualization and rejection disabled, preserved the channel-by-sample shape, and populated `EEG.etc.GEDAI` metrics and output-reference provenance.
- **Limits:** The cited work is a preprint and the bounded run establishes executability, not superiority, artifact-removal validity, leakage safety, or transfer to a new endpoint. Precomputed/interpolated/template modes are not participant-specific MRI/BEM evidence. Auto thresholds and ENOVA-based removal are fitted/adaptive operations. The noncommercial license and pending-patent notice constrain reuse; verify current terms. Plugin registry and upstream versions, wrapper behavior, dependencies, reference/rank effects, and computational defaults can change.
